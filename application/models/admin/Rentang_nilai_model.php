...
class Rentang_nilai_model extends CI_Model{

    public function queryById($data = [])
    {
        $this->db->select('*');
        $this->db->from('rentang_nilai');
        $this->db->where('id_rentang_nilai',$data['id_rentang_nilai']);
        $query = $this->db->get();
       return $query->row_array();
    }

    public function getRentangNilai(){
        $this->db->select('*');
        $this->db->from('rentang_nilai');
    ...
    public function buatQuery($tabel,$select_column = [],$order_column = [])
    {
        ...
        {
            $this->db->like('predikat_rentang_nilai',$_POST['search']['value']);
        }
        ...
        if(isset($_POST['order']))
        {
            ...
            $this->db->order_by('predikat_rentang_nilai', 'DESC');
        }        
    ...
    public function hapus($data = []){
        $this->db->delete('rentang_nilai', $data);
        $status = $this->db->affected_rows();
        if ($status > 0) {
            return 1;
         } else {
