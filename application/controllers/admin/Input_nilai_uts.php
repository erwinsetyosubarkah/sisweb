...
class Input_nilai_uts extends CI_Controller{
    ...
    public function index()
    {
        ...
        if(isset($_GET['id_agenda_mengajar'])){        
            ...
            if($data['agenda_mengajar'] == null){
                redirect('akademik-input-nilai-uts');
                die;
            }
            $this->load->view('admin/templates/header',$data);
            $this->load->view('admin/input_nilai_uts/masukan_nilai_uts',$data); 
            $this->load->view('admin/templates/footer');
        }else{       

            $this->load->view('admin/templates/header',$data);
            $this->load->view('admin/input_nilai_uts/input_nilai_uts',$data); 
            $this->load->view('admin/templates/footer');
        }
    }

 
    public function inputNilaiUts()
    {
        $data = $_POST['nilai'];
        
        $tabel = 'nilai';
        
        $this->load->model('admin/Input_nilai_uts_model');       
        $status = $this->Input_nilai_uts_model->inputNilaiUts($data,$tabel);
    ...
    public function queryAll()
    {
        ...
        $this->load->model('admin/Agenda_mengajar_model');
        $fetch_data = $this->Agenda_mengajar_model->buatDataTables($tabel,$select_column,$order_column,$where,$from);
        $data = [];
        $nourut = 1;
        foreach($fetch_data as $row)
        {
            ...
            $sub_array[] = '
                            <form action="akademik-input-nilai-uts" method="get">
                            <input type="hidden" name="id_agenda_mengajar" value="'.$row->id_agenda_mengajar.'">
                            <button type="submit" class="badge badge-primary badge-sm border-0" style="margin: 2px;"title="Input Nilai UTS"><i class="fa fa-fw fa-list"></i></button>
                            </form>
                            ';
            $data[] = $sub_array;
        ...
        $output = [
            ...
            'recordsTotal'      =>  $this->Agenda_mengajar_model->getAllData($tabel,$where,$from),
            'recordsFiltered'   =>  $this->Agenda_mengajar_model->getFilteredData($tabel,$select_column,$order_column,$where,$from),
            'data'              =>  $data
        ];
    ...
    public function queryAllSiswaKelas()
    {
        ...
        $select_column = [              
            ...
            'agenda_mengajar.keterangan_agenda_mengajar',                         
            'nilai.id_nilai',                         
            'nilai.pengetahuan_nilai',                       
            'nilai.keterampilan_nilai'                         
        ...
        $this->load->model('admin/Input_nilai_uts_model');
        $fetch_data = $this->Input_nilai_uts_model->buatDataTables($tabel,$select_column,$order_column,$where);
        $data = [];
        $nourut = 1;
        foreach($fetch_data as $row)
        {
            ...
            $sub_array[] = '
                            ...
                            <input type="number" name="pengetahuan_nilai" value="'.$row->pengetahuan_nilai.'" class="text-center">
                            
                            ';
            $sub_array[] = ' 
                            <input type="number" name="keterampilan_nilai" value="'.$row->keterampilan_nilai.'" class="text-center">
        ...
        $output = [
            'draw'              =>  intval($_POST['draw']),
            'recordsTotal'      =>  $this->Input_nilai_uts_model->getAllData($tabel,$where),
            'recordsFiltered'   =>  $this->Input_nilai_uts_model->getFilteredData($tabel,$select_column,$order_column,$where),
            'data'              =>  $data
        ];
