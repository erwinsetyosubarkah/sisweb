...
class Input_nilai_rapor extends CI_Controller{
    ...
    public function index()
    {
        ...
        if(isset($_GET['id_agenda_mengajar'])){        
            ...
            if($data['agenda_mengajar'] == null){
                redirect('akademik-input-nilai-rapor');
                die;
            }
            $this->load->view('admin/templates/header',$data);
            $this->load->view('admin/input_nilai_rapor/masukan_nilai_rapor',$data); 
            $this->load->view('admin/templates/footer');
        }else{       

            $this->load->view('admin/templates/header',$data);
            $this->load->view('admin/input_nilai_rapor/input_nilai_rapor',$data); 
            $this->load->view('admin/templates/footer');
        }
    }
    ...
    public function inputNilaiRapor()
    {
        ...
        $this->load->model('admin/Input_nilai_rapor_model');       
        $status = $this->Input_nilai_rapor_model->inputNilaiRapor($data,$tabel);
    ...
    public function queryAll()
    {
        $where = [
                    ...
                    "hari_agenda_mengajar" => $this->input->post('hari_agenda_mengajar')
        ...
        foreach($fetch_data as $row)
        {
            ...
            $sub_array[] = '
                            <form action="akademik-input-nilai-rapor" method="get">
                            <input type="hidden" name="id_agenda_mengajar" value="'.$row->id_agenda_mengajar.'">
                            <button type="submit" class="badge badge-primary badge-sm border-0" style="margin: 2px;"title="Input Nilai Rapor"><i class="fa fa-fw fa-list"></i></button>
                            </form>
                            ';
            $data[] = $sub_array;
    ...
    public function queryAllSiswaKelas()
    {
        ...
        $this->load->model('admin/Input_nilai_rapor_model');
        $fetch_data = $this->Input_nilai_rapor_model->buatDataTables($tabel,$select_column,$order_column,$where);
        $data = [];
        $nourut = 1;
        foreach($fetch_data as $row)
        {
        ...
        $output = [
            'draw'              =>  intval($_POST['draw']),
            'recordsTotal'      =>  $this->Input_nilai_rapor_model->getAllData($tabel,$where),
            'recordsFiltered'   =>  $this->Input_nilai_rapor_model->getFilteredData($tabel,$select_column,$order_column,$where),
            'data'              =>  $data
        ];
