...
class Cetak_rapor extends CI_Controller{
    ...
    public function index()
    {
        ...
        $data['pengaturan'] = $this->Pengaturan_model->getPengaturanById();   
            ...
            $this->load->view('admin/cetak_rapor/cetak_rapor',$data); 
    ...
    public function queryAllSiswaKelas()
    {
        ...
        $this->load->model('admin/Cetak_rapor_model');
        $fetch_data = $this->Cetak_rapor_model->buatDataTables($tabel,$select_column,$order_column,$where);
        $data = [];
        $nourut = 1;
        foreach($fetch_data as $row)
        {
            ...
            $sub_array[] = '
                    <form action="print-rapor" method="get" target="_blank">
                    ...
                    <button type="submit" class="badge badge-primary badge-sm border-0" style="margin: 2px;"title="Cetak Rapor"><i class="fa fa-fw fa-print"></i></button>
                    </form>
                    ';
            $data[] = $sub_array;
        ...
        $output = [
            ...
            'recordsTotal'      =>  $this->Cetak_rapor_model->getAllData($tabel,$where),
            'recordsFiltered'   =>  $this->Cetak_rapor_model->getFilteredData($tabel,$select_column,$order_column,$where),
            'data'              =>  $data
        ];
    ...
    public function cetakRapor(){   
       ...
       $this->load->model('admin/Cetak_rapor_model');
       $data['nilai_siswa'] = $this->Cetak_rapor_model->queryNilai($_GET['id_kelas'],$_GET['id_siswa']);
       $data['predikat'] = $this->Cetak_rapor_model->queryPredikat();
       $data['nilai_exkul'] = $this->Cetak_rapor_model->queryExtrakurikuler($_GET['id_siswa']);
       $data['wali_kelas'] = $this->Cetak_rapor_model->queryGuru($kelas['id_guru']);
       $data['tanggal'] = $this->Cetak_rapor_model->tanggalIndonesia(date("Y-m-d"));

       $this->load->view('admin/cetak_rapor/print_rapor',$data);
