          ...
          <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown no-arrow">
              <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                ...
                <span class="mr-2 d-none d-lg-inline text-gray-600 small"><?= $_SESSION['user']['nama_'.$level] ?></span>    
                <?php if($_SESSION['user']['foto_'.$level] == "" ): ?>            
                  <img class="img-profile rounded-circle" src="<?= base_url(); ?>assets/img/foto/noimage.png">
                <?php else :?>
                    <?php if(file_exists('./assets/img/foto/'.$_SESSION['user']['foto_'.$level])): ?>
                       <img class="img-profile rounded-circle" src="<?= base_url(); ?>assets/img/foto/<?= $_SESSION['user']['foto_'.$level]; ?>">
                    <?php else: ?>
                       <img class="img-profile rounded-circle" src="<?= base_url(); ?>assets/img/foto/noimage.png">
                    <?php endif; ?>
                <?php endif; ?>
